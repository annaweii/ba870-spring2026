...
def fetch_company_data(ticker: str):
    ...
    feature_dict = {
        ...
        "ni": ni, "sale": sale, "revt": sale, "oancf": oancf, "xint": xint,
...
def get_risk_label(prob):
    if prob >= THRESHOLD * 1.3:   return "High Risk",      "badge-high",     "banner-high"
    elif prob >= THRESHOLD * 0.8: return "Moderate Risk",  "badge-moderate", "banner-moderate"
    else:                         return "Low Risk",        "badge-low",      "banner-low"
...
# ==============================================================================
# PAGE 2 — COMPANY RISK CHECK (includes risk drivers)
# ==============================================================================
elif page == "Company Risk Check":
    st.markdown("""
    <div class="risk-header">
        <h1>Company Risk Check</h1>
        <p>Enter a ticker to generate a full distress risk profile</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    if run and ticker:
        ...
        for key, (label, ref, is_pct, higher_is_worse) in benchmarks.items():
            ...
            rows.append({
                "Feature": label, "Value": fmt(val, pct=is_pct),
                "Benchmark": fmt(ref, pct=is_pct), "Signal": signal,
                "_color": color, "_val": val, "_ref": ref,
            })
        ...
        with right:
            for _, row in df_drivers.iterrows():
                st.markdown(
                    f'<div class="card" style="border-left:4px solid {row["_color"]}; padding:0.7rem 1.1rem; margin-bottom:0.5rem;">'
                    f'<b style="color:#1a202c;">{row["Feature"]}</b><br>'
                    f'<span style="color:#718096; font-size:0.84rem;">Value: {row["Value"]} &nbsp;|&nbsp; Benchmark: {row["Benchmark"]}</span><br>'
                    f'<span style="color:{row["_color"]}; font-weight:700; font-size:0.84rem;">{row["Signal"]}</span>'
                    f'</div>',
                    unsafe_allow_html=True
                )
        ...
        narrative  = ""
        if increasing: narrative += f"Risk is negatively impacted by: {', '.join(increasing)}. "
        if reducing:   narrative += f"Positive signals include: {', '.join(reducing)}."
...
# ==============================================================================
    ...
    st.markdown(f"### {company_name}")
    st.markdown(
        f"<span style='color:#718096; font-size:0.9rem;'>Current risk classification: "
        f"<span class='{badge_class}'>{risk_label}</span> &nbsp;|&nbsp; "
        f"Distress probability: {prob*100:.1f}%</span>",
        unsafe_allow_html=True
    )
